;(function webpackUniversalModuleDefinition(root, factory) {
  ...
  else if (typeof exports === 'object') exports['Sketchfab'] = factory()
  else root['Sketchfab'] = factory()
})(this, function() {
  return /******/ (function(modules) {
    ...
    /******/
    /******/ /******/ __webpack_require__.p = '/static/builds/web/dist/' // Load entry module and return exports
    /******/
    ...
    /******/ [
      ...
      /***/ function(module, exports, __webpack_require__) {
        module.exports =
          ...
          !__webpack_require__(9)(function() {
            return (
              Object.defineProperty(__webpack_require__(32)('div'), 'a', {
                get: function() {
                  return 7
                },
              }).a != 7
      ...
      /***/ function(module, __webpack_exports__, __webpack_require__) {
        ...
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_keys__ = __webpack_require__(
          27
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_keys___default = __webpack_require__.n(
          __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_keys__
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_babel_runtime_helpers_typeof__ = __webpack_require__(
          33
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_babel_runtime_helpers_typeof___default = __webpack_require__.n(
          __WEBPACK_IMPORTED_MODULE_1_babel_runtime_helpers_typeof__
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_apis_viewer_lib_APIClient__ = __webpack_require__(
          72
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_utilities_ParseQueryString__ = __webpack_require__(
          73
        )

        window.SketchfabAPIClient =
          __WEBPACK_IMPORTED_MODULE_2_apis_viewer_lib_APIClient__['a' /* default */]
        ...
        var Sketchfab = function Sketchfab(version, target) {
          ...
          if (
            (typeof version === 'undefined'
              ? 'undefined'
              : __WEBPACK_IMPORTED_MODULE_1_babel_runtime_helpers_typeof___default()(version)) ===
            'object'
          ) {
            t = version
            v = null
          ...
          this._version = v
          this._target = t
          // get a unique identifier for the iframe
          // making sure we get message for us
          // and not another API sketchfab in the same page
          if (!window.sketchfabAPIinstances) window.sketchfabAPIinstances = []
          window.sketchfabAPIinstances.push(this)
          // if people keep adding/removing iframes
          // we might have problems
          // this._apiId = window.frames.length;
          // so we only count skfb api instances
          // must be a string
          this._apiId = window.sketchfabAPIinstances.length.toString()
          if (this._target.id) {
            // preferred id are user provided id
            // chances are big that an uniq Id exists
            // as it's DOM mandatory
            // but we concatenate just in case.
            // now apiId are Stable and easily debugged
            this._apiId += '_' + this._target.id
          }
        ...
        Sketchfab.prototype = {
          ...
          _getURLOption: function _getURLOption(name, defaultValue) {
            ...
            if (defaultValue === undefined || defaultValue === null) {
              defaultValue = dictQueryParam.default
            }
          ...
          _getURLOptions: function _getURLOptions() {
            ...
            var urlOptions = Object(
              __WEBPACK_IMPORTED_MODULE_3_utilities_ParseQueryString__['a' /* parse */]
            )(window.location.search)
            var namespace = 'skfb_'
            for (var key in urlOptions) {
              if (!key.startsWith(namespace)) continue
              this._transmitOptions[key.substr(5)] = urlOptions[key]
            }

            return this._optionsLoaded(urlOptions)
          },

          getEmbedURL: function getEmbedURL(urlid, options) {
            var url = this._url + '?api_version=' + this._version + '&api_id=' + this._apiId
            ...
            var transmitOptions = this._transmitOptions
            __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_keys___default()(
              this._transmitOptions
            ).forEach(function(key) {
              url += '&' + key.toString() + '=' + transmitOptions[key].toString()
            })
          ...
          reload: function reload(urlDomain) {
            var s = document.createElement('script')
            s.setAttribute(
              'src',
              'https://static.' + urlDomain + '/api/sketchfab-viewer-' + this._version + '.js'
            )
            s.addEventListener(
              'load',
              function() {
                this._url = this._urlTemplate.replace('YYYY', urlDomain)
                if (this._domain.indexOf('sketchfab.com') !== -1) {
                  this._transmitOptions.hook_prod = 1
                  this._transmitOptions.model = this._urlid
                }
                this._realInit()
              }.bind(this)
            )
          ...
          // ready+id <---- ready+id
          //          ----> api/initialize+id
          _initializeAPIEmbed: function _initializeAPIEmbed(e) {
            // filter message from everything
            // chrome extension, faye, other iframes
            if (!e.data) return
            if (!e.data.instanceId) return
            if (this._apiId !== e.data.instanceId) return
            if (e.data.type !== 'api.ready') return

            var opt = e.data.options
            if (opt && opt.domain) {
              this.reload(opt.domain)
              return
            }
            ...
            // wait til we get an acutal message
            // in order to be sure contentWindow exists
            // and has the correct domain in case of reloading
            var domainSource = this._target.src.split('/')
            domainSource = 'https://' + domainSource[2]
            //this._domain = domainSource;
            this._client = new window.SketchfabAPIClient(
            ...
            window.removeEventListener('message', this._claimEmbedBinded)
          },

          _realInit: function _realInit() {
            // special listener just for ready and instance id recuperation
      ...
      /***/ function(module, exports, __webpack_require__) {
        ...
        $Symbol[PROTOTYPE][TO_PRIMITIVE] ||
          __webpack_require__(3)($Symbol[PROTOTYPE], TO_PRIMITIVE, $Symbol[PROTOTYPE].valueOf)
      ...
      /***/ function(module, __webpack_exports__, __webpack_require__) {
        ...
        var APIClient = function APIClient(target, apiId, domain) {
          this._target = target
        ...
        APIClient.prototype = {
          ...
          setIdentifier: function setIdentifier(id) {
            this._instanceId = id
          },
          ...
          listenServer: function listenServer() {
            // Accept only msg of this data.type
            var msgTypeFilter = [
              //'api.ready',
            ...
            window.addEventListener(
              ...
              function(e) {
                // filter message from everything
                // chrome extension, faye, other iframes
                if (e.origin !== this._domain || !e.data || !e.data.type || !e.data.instanceId)
                  return

                // check if message is from this API sketchfab and from its instancied iframe
                if (e.data.instanceId !== this.getIdentifier()) return

                // check if we handle those type of msg
                var msgType = e.data.type
                if (msgTypeFilter.indexOf(msgType) === -1) return

                // if it's an event dont check the request
                if (msgType === 'api.event') {
                  ...
                  if (this._eventListeners['*'] || this._eventListeners.all) {
                    ;['*', 'all'].forEach(function(eventAll) {
                      var evts = this._eventListeners[eventAll]
                      if (!evts) return
                      evts.forEach(function(callback) {
                  ...
                  var evt = this._eventListeners[eventName]
                  if (evt) {
                  ...
                  var pending = this._pendingRequests[e.data.requestId]
                  if (!pending) return
                  pending.apply(null, e.data.results)
                }
              }.bind(this)
            )
          },
      ...
      /***/ function(module, __webpack_exports__, __webpack_require__) {
        ...
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_typeof__ = __webpack_require__(
          33
        )
        /* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_typeof___default = __webpack_require__.n(
        ...
        function normalize(parameters) {
          ...
          __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_keys___default()(
            ...
            destination[name] = !Array.isArray(parameters[name])
              ? [parameters[name]]
              : parameters[name]
          })
...
//# sourceMappingURL=sketchfab-viewer-1.3.1.js.map
